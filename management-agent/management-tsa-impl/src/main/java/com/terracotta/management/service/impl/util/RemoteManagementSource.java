...
public class RemoteManagementSource {
  ...
  public <T extends Representable> Collection<T> postToRemoteL2(String serverName, URI uri, Class<T> type) throws ManagementSourceException {
    String serverUrl = localManagementSource.getRemoteServerUrls().get(serverName);
    return resource(UriBuilder.fromUri(serverUrl).uri(uri).build())
        .post(new CollectionOfRepresentableGenericType<T>(type));
  }

  private WebResource.Builder resource(URI uri) {
    WebResource resource = client.resource(uri);
    resource.setProperty(ClientConfig.PROPERTY_CONNECT_TIMEOUT, (int)timeoutService.getCallTimeout());
    resource.setProperty(ClientConfig.PROPERTY_READ_TIMEOUT, (int)timeoutService.getCallTimeout());
    ...
    Object connectTimeout = client.getProperties().get(ClientConfig.PROPERTY_CONNECT_TIMEOUT);
    if (connectTimeout != null) {
      builder = builder.header(CONNECTION_TIMEOUT_HEADER_NAME, connectTimeout);
    }

    Object readTimeout = client.getProperties().get(ClientConfig.PROPERTY_READ_TIMEOUT);
    if (readTimeout != null) {
      builder = builder.header(READ_TIMEOUT_HEADER_NAME, readTimeout);
    }
