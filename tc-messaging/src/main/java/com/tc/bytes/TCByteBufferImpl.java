...
public class TCByteBufferImpl implements TCByteBuffer {
  ...
  private final Reference<ByteBuffer>      root;
  
  TCByteBufferImpl(boolean direct, ReferenceQueue<ByteBuffer> poolQueue) {
    ByteBuffer usable = null;

    if (poolQueue != null) {
      Reference<? extends ByteBuffer> ref = poolQueue.poll();
      if (ref != null) {
        usable = ref.get();
      }
    }
    ...
    if (usable != null) {
      Assert.assertTrue(usable.clear().capacity() == FIXED_BUFFER_SIZE);
      root = null;
    } else {
      if (direct) {
        usable = ByteBuffer.allocateDirect(FIXED_BUFFER_SIZE);
      } else {
        usable = ByteBuffer.allocate(FIXED_BUFFER_SIZE);
      }
      root = new SoftReference<>(usable, poolQueue);
    }
  ...
  TCByteBufferImpl(boolean direct, int capacity) {
    buffer = (direct) ?
