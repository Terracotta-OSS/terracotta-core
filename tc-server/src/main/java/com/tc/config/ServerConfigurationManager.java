...
public class ServerConfigurationManager implements PrettyPrintable {
  ...
  public boolean isRelaySource() {
    try (LockedConfiguration config = configurationProvider.getLockedConfiguration()) {
      return config.isRelaySource();
    }
  }
  
  public boolean isRelayDestination() {
    try (LockedConfiguration config = configurationProvider.getLockedConfiguration()) {
      return config.isRelayDestination();
    }
  }
  
  public InetSocketAddress getRelayPeer() {
    try (LockedConfiguration config = configurationProvider.getLockedConfiguration()) {
      return config.getRelayPeer();
    }
  }
  ...
  private static class LockedConfiguration implements Configuration, Closeable {
    ...
    @Override
    public boolean isRelaySource() {
      return delegate.isRelaySource();
    }

    @Override
    public boolean isRelayDestination() {
      return delegate.isRelayDestination();
    }

    @Override
    public InetSocketAddress getRelayPeer() {
      return delegate.getRelayPeer();
    }
  } 
