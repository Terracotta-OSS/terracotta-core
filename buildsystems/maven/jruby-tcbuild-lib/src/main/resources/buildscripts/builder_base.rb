...
class TerracottaBuilder
    ...
    def initialize(default_target, arguments)
        ...
        @start_time = Time.now
        # TODO: pass this in
        @basedir = $basedir.absolute_path #FilePath.new(File.dirname(File.expand_path(__FILE__)), "..").canonicalize
...
#        sfcs = StandardFileConfigSource.new('build-config')
        ecs = EnvironmentConfigSource.new(@ant)
