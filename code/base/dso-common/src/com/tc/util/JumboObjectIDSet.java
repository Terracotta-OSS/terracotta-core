 ...
 */
class JumboObjectIDSet extends AbstractSet implements SortedSet, PrettyPrintable, Externalizable {
  ...
  public JumboObjectIDSet() {
    ranges = new AATreeSet();
  }

  public JumboObjectIDSet(Collection c) {
    this();
    if (c instanceof JumboObjectIDSet) {
      JumboObjectIDSet other = (JumboObjectIDSet) c;
      // fast way to clone
  ...
  public String toString() {
    StringBuffer sb = new StringBuffer("JumboObjectIDSet " + getCompressionDetails() + "[");
    for (Iterator i = ranges.iterator(); i.hasNext();) {
  ...
  private class ObjectIDSetIterator implements Iterator {
    ...
    public void remove() {
      ...
      if (expectedModCount != modCount) throw new ConcurrentModificationException();
      JumboObjectIDSet.this.remove(lastReturned);
      expectedModCount = modCount;
