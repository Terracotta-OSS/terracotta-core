...
final class BitSetObjectIDSet extends ObjectIDSetBase {
  ...
  public BitSetObjectIDSet(Collection c) {
    super();
    if (c instanceof BitSetObjectIDSet) {
      BitSetObjectIDSet other = (BitSetObjectIDSet) c;
      // fast way to clone
  ...
  public boolean add(ObjectID id) {
    ...
    BitSet prev = (BitSet) this.ranges.find(new BitSet(start, 0));
    if (prev != null) {
    ...
    boolean isAdded = this.ranges.insert(newRange);
    if (isAdded) {
      this.size++;
      this.modCount++;
    }
  ...
  public boolean remove(ObjectID id) {
    ...
    BitSet current = (BitSet) this.ranges.find(new BitSet(start, 0));
    if (current == null) {
      // Not found
      return false;
    }
  ...
  public boolean contains(ObjectID id) {
    long lid = id.toLong();
    long start = calculateStart(lid);
    BitSet r = (BitSet) this.ranges.find(new BitSet(start, 0));
    if (r == null) {
      return false;
    } else {
  ...
  public Object deserializeFrom(TCByteBufferInput in) throws IOException {
    ...
    while (_size > 0) {
      ...
      long nextRanges = in.readLong();
      BitSet r = new BitSet(start, nextRanges);
  ...
  public ObjectID first() {
    ...
    BitSet min = (BitSet) this.ranges.findMin();
  ...
  public ObjectID last() {
    if (this.size == 0) { throw new NoSuchElementException(); }
    BitSet max = (BitSet) this.ranges.findMax();
    return new ObjectID(max.last());
  }

  private long calculateStart(long lid) {
    if (lid < 0) {
  ...
  private class BitSetObjectIDSetIterator implements Iterator {
    ...
    public void remove() {
      ...
      if (this.expectedModCount != BitSetObjectIDSet.this.modCount) { throw new ConcurrentModificationException(); }
       ...
       long oid = this.lastReturned.toLong();
  ...
  private static class BitSet extends AANode implements Cloneable, Comparable {
    ...
    public long size() {
      return (Long.bitCount(nextLongs)); // since it is all inclusive
    }
