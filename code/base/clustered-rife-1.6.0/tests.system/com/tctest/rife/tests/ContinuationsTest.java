...
public class ContinuationsTest extends AbstractTwoServerDeploymentTest {
  ...
  /**
   * This test simply counts to 10 inside a while loop. The execution is paused in the middle of the loop so that the
   * user can press a button on the form to increase the count. This test will migrate continuations between servers and
   * check if earlier continuations are properly isolated from newer ones.
   */
  ...
  /**
   * This test uses pause() and stepback() on the server to simulate a loop, it will continue until the total counter
   * reaches 50 the counter starts at 0 and will increase by the value that is sent through the form.
   */
  ...
  private WebResponse doQueryStringRequest(WebApplicationServer server, String location, WebForm form)
      throws MalformedURLException, IOException, SAXException {
    return server.ping(location + (null == form ? "" : "?" + form.newUnvalidatedRequest().getQueryString()));
  }

  /**
   * This test contains operations on all basic JDK types as member fields, static fields and local variables in an
   * element that uses continuations. It switches between servers to ensure that all the types are correctly propagated
   * through the shared roots.
   */
  ...
  /**
   * This test checks if continuations also work over a cluster when the element doesn't extend Element, but rather
   * implements the ElementAware interface.
   */
  ...
  /**
   * This test checks if call/answer continuations work correctly over several nodes.
   */
  ...
  private static class ContinuationsTestSetup extends TwoServerTestSetup {
    ...
    protected void configureWar(DeploymentBuilder builder) {
      builder.addDirectoryOrJARContainingClass(com.uwyn.rife.Version.class) // rife jar
          .addFilter("RIFE", "/*", RifeFilter.class, new HashMap() {
            {
              put("rep.path", "rife-config-files/continuations/participants.xml");
            }
          }).addResource("/web-resources", "counter.html", "WEB-INF/classes").addResource("/web-resources",
                                                                                          "stepback.html",
                                                                                          "WEB-INF/classes");

      if (TestConfigObject.getInstance().appServerId() == AppServerInfo.JETTY) {
