...
public class ModulesPanel extends XContainer implements TableModelListener {
  ...
  private ModulesEventListener   m_changeListener;
  private ModulesEventListener   m_setDirtyListener;
  ...
  public void load(ContainerResource containerRes) {
    ...
    m_moduleAddListener = new ActionListener() {
      public void actionPerformed(ActionEvent ae) {
        TcPlugin.getStandardDisplay().asyncExec(new Runnable() {
          public void run() {
            Shell shell = TcPlugin.getStandardDisplay().getActiveShell();
            AddModuleDialog dialog = new AddModuleDialog(shell, "Module Declaration", "Module Declaration");
    ...
    m_repoAddListener = new ActionListener() {
      public void actionPerformed(ActionEvent ae) {
        TcPlugin.getStandardDisplay().asyncExec(new Runnable() {
          public void run() {
            Shell shell = TcPlugin.getStandardDisplay().getActiveShell();
            RepoLocationDialog dialog = new RepoLocationDialog(shell, "Repository Declaration",
  ...
  synchronized void addChangeListener(ModulesEventListener listener) {
    m_changeListener = listener;
  }

  synchronized void addSetDirtyListener(ModulesEventListener listener) {
    m_setDirtyListener = listener;
  }

  private synchronized void fireChangeEvent() {
    if (m_changeListener != null) m_changeListener.handleEvent();
  }

  private synchronized void fireSetDirtyEvent() {
    if (m_setDirtyListener != null) m_setDirtyListener.handleEvent();
  }
  ...
  public static interface ModulesEventListener {
    void handleEvent();
  }
}
