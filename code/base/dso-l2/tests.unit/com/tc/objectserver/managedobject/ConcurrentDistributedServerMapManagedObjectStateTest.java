...
public class ConcurrentDistributedServerMapManagedObjectStateTest extends AbstractTestManagedObjectState {
  ...
  public void testDehydration() throws Exception {
    ...
    state2.apply(new ObjectID(1), cursor2, new BackReferences());
    assertEquals(state, state2);

    dnaWriter = new TestDNAWriter();
    state.dehydrate(new ObjectID(1), dnaWriter, DNAType.L1_FAULT); // Only dehydrate the fields
    Assert.assertEquals(2, dnaWriter.getActionCount()); // only the physical fields are dehydrated
    ...
    final ConcurrentDistributedServerMapManagedObjectState state3 = (ConcurrentDistributedServerMapManagedObjectState) createManagedObjectState(
                                                                                                                                                CDSM_DSO_CLASSNAME,
                                                                                                                                                cursor3);
    state3.apply(new ObjectID(1), cursor3, new BackReferences());
