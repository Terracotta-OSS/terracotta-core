...
public class ConfigPropertiesTest extends TestCase {
  ...
  private static final TCLogger logger             = TCLogging.getTestingLogger(ConfigPropertiesTest.class);
  ...
  public void testConfigProperties() {
    try {
      new ConfigProperties(null, TCPropertiesImpl.getProperties(), logger);
      new ConfigProperties(new MockWebAppConfig(), TCPropertiesImpl.getProperties(), logger);
    } catch (Exception e) {
  ...
  public void testSystemPropertyOverrides() {
    ConfigProperties cp = new ConfigProperties(wac, tpc, logger);
  ...
  public void testEmptyProperties() {
    ConfigProperties cp = new ConfigProperties(wac, TCPropertiesImpl.getProperties(), logger);
  ...
  public void testDefaults() {
    ConfigProperties cp = new ConfigProperties(null, TCPropertiesImpl.getProperties(), logger);
  ...
  public void testGetSessionListeners() {
    ...
    final ConfigProperties cp = new ConfigProperties(null, new MockTCPropeties(props), logger);
  ...
  public void testMakeSessionListener() {
    ...
    HttpSessionListener listener = cp.makeSessionListener("com.some.Bogus.Type");
    assertNull(listener);

    listener = cp.makeSessionListener(ConfigPropertiesTest.SessionListener.class.getName());
    assertTrue(listener instanceof ConfigPropertiesTest.SessionListener);

    listener = cp.makeSessionListener(ConfigPropertiesTest.AttributeListener.class.getName());
    assertNull(listener);
  }
  ...
  public void testMakeAttributeListener() {
    ...
    HttpSessionAttributeListener listener = cp.makeAttributeListener("com.some.Bogus.Type");
    assertNull(listener);

    listener = cp.makeAttributeListener(ConfigPropertiesTest.AttributeListener.class.getName());
    assertTrue(listener instanceof ConfigPropertiesTest.AttributeListener);

    listener = cp.makeAttributeListener(ConfigPropertiesTest.SessionListener.class.getName());
    assertNull(listener);
  }
