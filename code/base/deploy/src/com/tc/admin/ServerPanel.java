...
public class ServerPanel extends XContainer {
  ...
  /**
   * The only differences between activated() and started() is the status message
   * and the serverlog is only added in activated() under the presumption that
   * a non-active server won't be saying anything.
   */
  void started() {
    m_hostField.setEditable(false);
    m_portField.setEditable(false);
  ...
  private void showRuntimeInfo() {
    ...
    if(clusterMembers.length > 1) {
      ...
      if(m_altProductInfoPanel != null &&
         (parent = (Container)m_altProductInfoPanel.getParent()) != null)
      {
  ...
  void addClusterMember(L2Info clusterMember) {
    ...
    if(host.equals("localhost") || host.equals(L2Info.IMPLICIT_L2_NAME)) {
      clusterMember =
        new L2Info(clusterMember.name(), m_serverNode.getHost(), clusterMember.jmxPort());
    }
    
    ServerConnectionManager scm =
      new ServerConnectionManager(clusterMember, true, m_clusterMemberListener);
