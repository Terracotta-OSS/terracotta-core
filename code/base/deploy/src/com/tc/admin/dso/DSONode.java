...
public class DSONode extends ComponentNode {
  public DSONode(ConnectionContext cc) {
    ...
    statName = "CacheHitRatio";
    header   = acc.getMessage("dso.cache.hit.ratio");
    yLabel   = acc.getMessage("dso.cache.hit.ration.range.label");
    panel    = new DoubleRatePanel(cc, bean, statName, header, xLabel, yLabel);
    node     = new PollerNode(header, panel);

    panel.getRangeAxis().setRange(0d, 1d);
