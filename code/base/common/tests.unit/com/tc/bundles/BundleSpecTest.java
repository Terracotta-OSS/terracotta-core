...
public class BundleSpecTest extends TestCase {
  ...
  public void testGetRequirementsString() throws BundleException {
    ...
    reqs = check(1, "org.terracotta.modules.clustered_surefire_2.3;bundle-version:=1.0.0.SNAPSHOT");
    ...
    assertEquals("1.0.0.SNAPSHOT", reqs[0].getVersion());
    
    reqs = check(1, "org.terracotta.modules.excludes_config;bundle-version:=1.0.0.SNAPSHOT");
    assertEquals("org.terracotta.modules.excludes_config", reqs[0].getSymbolicName());
    assertEquals("org.terracotta.modules", reqs[0].getGroupId());
    assertEquals("excludes_config", reqs[0].getName());
    assertEquals("1.0.0.SNAPSHOT", reqs[0].getVersion());
    
    reqs = check(1, "org.terracotta.modules.clustered_cglib_2.1.3;bundle-version:=1.0.0.SNAPSHOT");
