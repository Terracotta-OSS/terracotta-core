...
public abstract class AbstractMap implements Map {
  ...
  protected AbstractMap() {
    // makes it so that only subclasses can call this
  }
   ...
   * and only if this map contains at least one mapping to a value <tt>v</tt> such that <tt>(value==null ?
   * v==null : value.equals(v))</tt>.
   * This operation will probably require time linear in the map size for most implementations of map.
   * <p>
   * This implementation iterates over entrySet() searching for an entry with the specified value. If such an entry is
   * found, <tt>true</tt> is returned. If the iteration terminates without finding such an entry, <tt>false</tt> is
   * returned. Note that this implementation requires linear time in the size of the map.
   * 
   ...
   * @throws NullPointerException this map does not permit <tt>null</tt> keys or values, and the specified key or
   *         value is <tt>null</tt>.
   */
   ...
   * <p>
   * This implementation iterates over <tt>entrySet()</tt> searching for an entry with the specified key. If such an
   * entry is found, its value is obtained with its <tt>getValue</tt> operation, the entry is is removed from the
   ...
   * <p>
   * Note that this implementation throws an <tt>UnsupportedOperationException</tt> if the <tt>entrySet</tt>
  ...
  /**
   * Copies all of the mappings from the specified map to this map (optional operation). These mappings will replace any
   ...
   * <p>
   * This implementation calls <tt>entrySet().clear()</tt>. Note that this implementation throws an
   * <tt>UnsupportedOperationException</tt> if the <tt>entrySet</tt> does not support the <tt>clear</tt>
   * operation.
   * 
   ...
   * results of the iteration are undefined.) The Set supports element removal, which removes the corresponding entry
   * from the map, via the Iterator.remove, Set.remove, removeAll retainAll, and clear operations. It does not support
   * the add or addAll operations.
   * <p>
   * This implementation returns a Set that subclasses AbstractSet. The subclass's iterator method returns a "wrapper
   * object" over this map's entrySet() iterator. The size method delegates to this map's size method and the contains
   * method delegates to this map's containsKey method.
   * <p>
   ...
   * <tt>removeAll</tt>,<tt>retainAll</tt> and <tt>clear</tt> operations. It does not support the <tt>add</tt>
   ...
   * size method and the contains method delegates to this map's containsValue method.
   * <p>
   ...
   * backed by the map, so changes to the map are reflected in the set, and vice-versa. (If the map is modified while an
   * iteration over the set is in progress, the results of the iteration are undefined.) The set supports element
   ...
   * same mappings if <tt>t1.keySet().equals(t2.keySet())</tt> and for every key <tt>k</tt> in <tt>t1.keySet()</tt>,
   * <tt> (t1.get(k)==null ? t2.get(k)==null :
   * t1.get(k).equals(t2.get(k))) </tt>. This ensures that the
   * <tt>equals</tt> method works properly across different implementations of the map interface.
   * <p>
   ...
   * entry in the map's <tt>entrySet()</tt> view. This ensures that <tt>t1.equals(t2)</tt> implies that
   * <tt>t1.hashCode()==t2.hashCode()</tt> for any two maps <tt>t1</tt> and <tt>t2</tt>, as required by the
   * general contract of Object.hashCode.
   * <p>
   * This implementation iterates over <tt>entrySet()</tt>, calling <tt>hashCode</tt> on each element (entry) in
  ...
  /**
   * Returns a string representation of this map. The string representation consists of a list of key-value mappings in
   * the order returned by the map's <tt>entrySet</tt> view's iterator, enclosed in braces (<tt>"{}"</tt>).
   ...
   * are converted to strings as by <tt>String.valueOf(Object)</tt>.
   * <p>
   ...
   * each entry except the last, the string <tt>", "</tt> is appended. Finally a right brace is appended. A string is
   * obtained from the stringbuffer, and returned.
   * 
  ...
  /**
   * This should be made public as soon as possible. It greately simplifies the task of implementing Map.
   */
