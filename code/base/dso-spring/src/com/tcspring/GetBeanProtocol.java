...
public class GetBeanProtocol {
  ...
  /**
   * Captures the name of the bean being created and makes it accessible to virtualizeSingletonBean() It also maintain
   * the locking for distributed bean initialization. Invoked around
   * <code>AbstractAutowireCapableBeanFactory.createBean(String, ..)</code> method.
   * 
  ...
  /**
   * Virtualize singleton bean. Invoked around call to <code>BeanWrapper.getWrappedInstance()</code> method within
  ...
  //Spring 2.5.x entry point
  public void initializeSingletonBean(String beanName, AbstractBeanDefinition mergedBeanDefinition,
                                      BeanWrapper instanceWrapper, AutowireCapableBeanFactory beanFactory) {
    
    initializeSingletonBean(beanName, (BeanDefinition)mergedBeanDefinition, instanceWrapper, beanFactory);
  }

  //Spring 2.0.x entry point
  ...
  /**
   * Initialize singleton bean. Invoked after call to <code>AbstractAutowireCapableBeanFactory.populateBean(..)</code>
   * method within execution of <code>AbstractAutowireCapableBeanFactory.createBean(String, ..))</code>
   * 
   * @see GetBeanProtocol#beanNameCflow(StaticJoinPoint, String, AutowireCapableBeanFactory)
   ...
   */
  private void initializeSingletonBean(String beanName, BeanDefinition mergedBeanDefinition,
                                      BeanWrapper instanceWrapper, AutowireCapableBeanFactory beanFactory) {
    if (beanFactory instanceof DistributableBeanFactory) {
      DistributableBeanFactory distributableBeanFactory = (DistributableBeanFactory) beanFactory;
      if (distributableBeanFactory.isDistributedSingleton(beanName)) {
