...
class JavaVersion
    ...
    def initialize(version_string)
        if version_string =~ /^\s*(\d+)\.(\d+)\.(\d+)(?:_(\d+))?\s*$/
            ...
            raise RuntimeError, "Version '#{version_string}' isn't a valid Java version"
        end
    end
...
class JVMSet
    ...
    def [](key)
        raise RuntimeError, "No JVM for '%s'." % key unless @jvms.has_key?(key)
        @jvms[key]
    end
...
class JVM
    ...
    def validate(descrip)
        unless FileTest.file?(java)
            raise RuntimeError, "The Java home for #{descrip}, at '#{@java_home}', has no 'java' executable at '#{@java_path}'. Please correct this problem and try again."
        end
        unless FileTest.file?(javac)
            raise RuntimeError, "The Java home for #{descrip}, at '#{@java_home}', has no 'javac' executable at '#{@javac_path}'. Please correct this problem and try again."
        end
    end
        ...
        unless @min_version.nil? && @max_version.nil?
            ...
            unless @min_version.nil?
                ...
                if this_version < required_version
                    raise RuntimeError, "The version of #{descrip}, '#{this_version}', is less than the minimum required of '#{required_version}'."
                end
            end
