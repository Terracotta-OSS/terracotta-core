...
public final class FastHashMapTestApp extends AbstractErrorCatchingTransparentApp {
  ...
  public FastHashMapTestApp(final String appId, final ApplicationConfig cfg, final ListenerProvider listenerProvider) {
  ...
  private void removeDataFromMap(final int count) {
    for (int pos = 0; pos < count; ++pos) {
      clusteredFastHashMap.remove(clusteredFastHashMap.keySet().iterator().next());
    }
  }
