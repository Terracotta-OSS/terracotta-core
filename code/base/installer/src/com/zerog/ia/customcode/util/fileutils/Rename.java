...
public class Rename extends CustomCodeAction {
  ...
  public void install(InstallerProxy ip) {
    /**
     * InstallAnywhere variable:RENAME_SUCCESS possible return values are: SUCCESS ERROR
     */
    ...
    /*
     * substitute() will return an empty string for any InstallAnywhere Variable that hasn't been assigned yet. /* If
     * there is both a source and a destination, copy the files.
     */

    if (target.equals("") || newName.equals("")) {
  ...
  /**
   * Rename the file represented by source to the file represented by destination.
   */
  public static void rename(File target, String newName) throws IOException {
    if (!target.renameTo(new File(target.getParent(), newName))) throw new IOException("Couldn't rename file.");
  }
