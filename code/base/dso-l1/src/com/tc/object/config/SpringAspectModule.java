...
public class SpringAspectModule implements AspectModule {
  ...
  private void buildDefinitionForBeanDefinitionProtocol(AspectModuleDeployer deployer) {
    ...
    builder.addAdvice(
                   "after",
                   "execution(* org.springframework.beans.factory.support.BeanDefinitionRegistry+.registerBeanDefinition(..)) AND args(beanName, beanDefinition)",
                   "saveBeanDefinition(StaticJoinPoint jp, java.lang.String beanName, org.springframework.beans.factory.config.BeanDefinition beanDefinition)");
    ...
    builder.addAdvice(
                   ...
                   "cflow(execution(* org.springframework.context.support.AbstractRefreshableApplicationContext+.loadBeanDefinitions(..))) "
                       ...
                       + "AND call(* org.springframework.util.ClassUtils.forName(..)) " + "AND args(className, loader)",
  ...
  private void buildDefinitionForGetBeanProtocol(AspectModuleDeployer deployer) {
    ...
    // + "AND this(factory)",
    // "registerBeanPostProcessor(StaticJoinPoint jp, org.springframework.beans.factory.support.AbstractBeanFactory
    // factory)");
  ...
  private void buildDefinitionForScopeProtocol(AspectModuleDeployer deployer) {
    if (hasClass("org.springframework.beans.factory.config.Scope", deployer)) {
      ...
      AspectDefinitionBuilder builder = deployer.newAspectBuilder("com.tcspring.ScopeProtocol",
                                                                  DeploymentModel.PER_JVM, null);
  ...
  private void buildDefinitionForApplicationContextEventProtocol(final AspectModuleDeployer deployer) {
    ...
    builder.addAdvice("around",
                      ...
                      "interceptEvent(StaticJoinPoint jp, " + "org.springframework.context.ApplicationEvent event, "
  ...
  private void buildDefinitionForAopProxyFactoryProtocol(final AspectModuleDeployer deployer) {
    if (hasClass("org.springframework.aop.framework.AopProxyFactory", deployer)) {
      ...
      AspectDefinitionBuilder builder = deployer.newAspectBuilder("com.tcspring.AopProxyFactoryProtocol",
                                                                  DeploymentModel.PER_JVM, null);
      ...
      builder.addAdvice("around", "execution(* org.springframework.aop.framework.AopProxyFactory+.createAopProxy(..)) "
