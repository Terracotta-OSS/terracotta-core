...
public class ClientObjectManagerImpl implements ClientObjectManager, PortableObjectProvider, Evictable, DumpHandler,
  ...
  private void markLookupInProgress(ObjectID id) {
  ...
  private synchronized void markCreateInProgress(ObjectID id, TCObject object) {
    ObjectLatchState ols = (ObjectLatchState) objectLatchStateMap.get(id);
  ...
  private TCObject lookup(ObjectID id, ObjectID parentContext, boolean noDepth) throws ClassNotFoundException {
    ...
    try {
      synchronized (this) {
        while (true) {
          ObjectLatchState olsForId = getObjectLatchState(id);       
          obj = basicLookupByID(id);
          //object exists in local cache
          if (obj != null) {
            return obj;   
          //check to see if the object is being created, if so
          //add to the wait set and return the object
          } else if (olsForId != null && olsForId.isCreateState()) {
            ObjectLatchState ols = getObjectLatchState(id);
            lookupContext.getObjectLatchWaitSet().add(ols);
            return ols.getObject();
          //check to see if the object is being looked up,
          //then just wait.
          } else if (olsForId != null && olsForId.isLookupState()) {
            try {
              wait();
            } catch (InterruptedException ie) {
              isInterrupted = true;
            }
          //otherwise, we need to lockup the object
          } else {
            retrieveNeeded = true;
