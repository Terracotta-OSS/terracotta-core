...
/**
 * The invalidator watches a portion of the CacheDataStore (a range of the submaps) and is run in a loop,
 * periodically running an eviction of all keys existent in this node.  Remote keys are ignored to minimize
 * faulting over the network.  
 * 
 * Additionally, at the beginning of each run, the cache attempts to become the "global evictor".  For 
 * each chunk of the cache, only one of the local evictors in the cluster will become the global evictor.
 * That thread is responsible for both its own local eviction and also the global eviction.  Global 
 * eviction entails checking "orphan" keys (those not currently faulted into any node's cache) for eviction.
 * The process of doing this will cause those keys to be loaded into this node such that they are no longer 
 * orphans.  
 * 
 * This class should be completely local and no state should be shared across the cluster.  This is done 
 * by indicating transient on the array holding these objects in the CacheDataStore.
 */
class CacheEntryInvalidator {
  ...
  private int                evictionCount;    // when this == globalEvictionFrequency, do global eviction
  private int                globalWaitCount;  // number of local eviction cycles we've waited during global eviction
  ...
  public CacheEntryInvalidator(GlobalKeySet globalKeySet, int startEvictionIndex, int lastEvictionIndex, 
    ...
    this.localInvalidationLock = new Lock(createLockName("tc:local_time_expiry_cache_invalidator_lock_", config.getCacheName(), startEvictionIndex), manager);
    this.globalInvalidationLock = new Lock(createLockName("tc:global_time_expiry_cache_invalidator_lock_", config.getCacheName(), startEvictionIndex), manager);
  }
  ...
  protected void evictLocalElements() {
    ...
    store.evictExpiredElements(this.startEvictionIndex, this.lastEvictionIndex);
    log("Local eviction finished");
  }

  /**
   * Attempt to obtain global invalidator write lock.
   * If successful, set isGlobalInvalidator to true
   */
