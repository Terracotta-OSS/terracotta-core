...
public class ConfigHelper {
  ...
  private static final String TC_INSTALL_DIR =
    SessionIntegratorFrame.getTCInstallDir();
  
  private static final String TOMCAT_SANDBOX =
    SessionIntegratorFrame.getSandBoxRoot();
  ...
  public boolean ensureWebApplication(String name) {
    ...
    if(dsoApp != null) {
      ...
      String[] appNames = apps.getWebApplicationArray();
      for(int i = 0; i < appNames.length; i++) {
        if(appNames[i].equals(name)) {
          return false;
        }
      }
      apps.addWebApplication(name);
