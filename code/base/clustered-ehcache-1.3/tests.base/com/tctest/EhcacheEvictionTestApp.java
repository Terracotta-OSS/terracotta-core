...
public class EhcacheEvictionTestApp extends AbstractErrorCatchingTransparentApp {
	...
	protected void runTest() throws Throwable {
		...
		if (index == 0) {
			// Even though the singleton field of CacheManager is a root, we
			// need to
			// set cacheManager field to be a root and remove the shutdown hook
			// of cache manager
			// before assigning to root. This hack is to bypass the shutdown
			// hook problem of 1.2.4
			// in system test.
		...
		if (index == 0) {
			// setting time to idle 10 times time to live to make sure that it
			// will not expire because of the idle time.
	...
	private void runSimplePut(int index) throws Throwable {
		...
		System.err.println("Time to put " + NUM_OF_CACHE_ITEMS
				+ " objects into the cache: " + (en - st) + " ms.");
	...
	private Cache addCache(final String name, long timeToIdleSeconds)
			throws Throwable {
		cacheManager.removeCache(name);
		Cache cache = new Cache(name, 2, MemoryStoreEvictionPolicy.LRU, false,
				null, false, timeToIdleSeconds * 2, timeToIdleSeconds, false,
				1, null);
	...
	private Cache addCache(final String name, long timeToLiveSeconds,
			long timeToIdleSeconds) throws Throwable {
		cacheManager.removeCache(name);
		Cache cache = new Cache(name, 2, MemoryStoreEvictionPolicy.LRU, false,
				null, false, timeToLiveSeconds, timeToIdleSeconds, false, 1,
				null);
