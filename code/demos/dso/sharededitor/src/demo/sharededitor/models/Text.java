...
public class Text extends BaseObject implements IFontable, ITexturable
{
	...
	private synchronized void renderText(FontRenderContext frc, boolean showCursor)
	{
		String text = this.text;
		if (showCursor || (text.length() == 0))
		{
		   if (text.length() > 0) text = this.isInverted ? text : text + "|";
		   else text = "_";
	   }
		
		AttributedString as = new AttributedString(text);
		as.addAttribute(TextAttribute.FONT, new Font(
			this.fontName,
			Font.PLAIN,
			this.fontSize));
	...
	public void draw(Graphics2D g, boolean showAnchors)
	{
		...
		if (showAnchors)
		{
			...
			Shape border     = new Rectangle2D.Double(bounds.x - 5, bounds.y - 5, bounds.width + 5, bounds.height + 5);
			if (this.isInverted)
			{
			   g.setXORMode(Color.LIGHT_GRAY);
   			g.fillRect(bounds.x - 5, bounds.y - 5, bounds.width + 5, bounds.height + 5);
			   g.setPaintMode();
		   }
