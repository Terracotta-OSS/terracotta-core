...
public abstract class AbstractClientBase implements Runnable {
	...
	public AbstractClientBase(String args[]) {
		...
		try {
			JMXConnector jmxConnector = JMXUtils.getJMXConnector("localhost",
					testControlMbeanPort);
			final MBeanServerConnection mbs = jmxConnector
					.getMBeanServerConnection();
			testControlMBean = MBeanServerInvocationProxy.newMBeanProxy(mbs,
					TestHandler.TEST_SERVER_CONTROL_MBEAN,
					TestHandlerMBean.class, false);
		} catch (Exception e) {
			System.out
					.println("****** Exception while connecting to TestMBean Port["
							+ testControlMbeanPort + "]");
