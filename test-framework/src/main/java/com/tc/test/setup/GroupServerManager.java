...
public class GroupServerManager {
	...
	private static final boolean DEBUG = Boolean
			.getBoolean("test.framework.debug");
	...
	private ExecutorService asyncExecutor = Executors
	...
	public GroupServerManager(GroupsData groupData, TestConfig testConfig,
			File tempDir, File javaHome, File tcConfigFile) throws Exception {
		...
		createServers();
		this.tcServerInfoMBeans = new TCServerInfoMBean[groupData
				.getServerCount()];
		...
		if (isProxyL2GroupPort()) {
			proxyL2Managers = new ProxyConnectManager[groupData
					.getServerCount()];
			...
			for (int i = 0; i < groupData.getServerCount(); ++i) {
				proxyL2Managers[i] = new ProxyConnectManagerImpl(
						...
						groupData.getProxyL2GroupPort(i));
		...
		if (isProxyDsoPort()) {
			proxyL1Managers = new ProxyConnectManager[groupData
					.getServerCount()];
			for (int i = 0; i < groupData.getServerCount(); ++i) {
				proxyL1Managers[i] = new ProxyConnectManagerImpl(
						groupData.getDsoPort(i), groupData.getProxyDsoPort(i));
	...
	private void createServers() {
		...
		for (int i = 0; i < groupData.getServerCount(); i++) {
			...
			if (isProxyL2GroupPort()) {
				perServerJvmArgs = testConfig.getL2Config()
						.getExtraServerJvmArgs();
				// hidden tc.properties only used by L2 proxy testing purpose
				perServerJvmArgs
						.add("-Dcom.tc."
								+ TCPropertiesConsts.L2_NHA_TCGROUPCOMM_RECONNECT_L2PROXY_TO_PORT
								+ "=" + groupData.getL2GroupPort(i));
			} else {
				perServerJvmArgs = testConfig.getL2Config()
						.getExtraServerJvmArgs();
			...
			serverControl[i] = getServerControl(groupData.getDsoPort(i),
					groupData.getJmxPort(i), groupData.getServerNames()[i],
					perServerJvmArgs);
			expectedServerRunning[i] = false;
		}
	}

	private ServerControl getServerControl(int dsoPort, int jmxPort,
			String serverName, List aJvmArgs) {
		File workingDir = new File(this.tempDir, serverName);
		workingDir.mkdirs();
		return new ExtraProcessServerControl(HOST, dsoPort, jmxPort,
				tcConfigFile.getAbsolutePath(), true, serverName, aJvmArgs,
				javaHome, true, workingDir);
	}

	public void startAllServers() throws Exception {
		if (activeIndex >= 0) {
			throw new AssertionError("Server(s) has/have been already started");
		}
		debugPrintln("***** startAllServers():  about to start ["
				+ serverControl.length + "]servers  threadId=["
		...
		debugPrintln("***** startAllServers():  about to search for active  threadId=["
				+ Thread.currentThread().getName() + "]");
	}
	...
	public synchronized void startServer(int index) throws Exception {
		...
		if (isProxyL2GroupPort()) {
			...
			debugPrintln("***** Caching tcServerInfoMBean for server=["
					+ serverControl[index].getDsoPort() + "]");
	...
	private void verifyIndex(int serverIndex) {
		Assert.assertTrue("serverIndex " + serverIndex
				+ " no. of servers per Group: " + groupData.getServerCount(),
		...
		while (index < 0) {
			...
			for (int i = 0; i < groupData.getServerCount(); i++) {
				...
				try {
					...
					System.out
							.println("Need to fetch tcServerInfoMBean for server=["
									+ serverControl[i].getDsoPort() + "]...");
					try {
						...
						System.out
								.println("exception restoring jmx connection ["
										+ e2.getMessage() + "]");
						continue;
					}
				}
				debugPrintln("********  index=[" + index + "]  i=[" + i
						+ "] active=[" + isActive + "] lastCrashedIndex=["
						+ lastCrashedIndex + "] threadId=["
						+ Thread.currentThread().getName() + "]");
				...
				if (isActive) {
					if (index < 0) {
						...
						throw new Exception(
								"More than one active server found.");
					}
				}
			}
			Thread.sleep(1000);
	...
	private void startL1Proxy(int index) {
		if (isProxyDsoPort()) {
			System.out
					.println("*** Starting the DSO proxy with proxy port as "
							+ proxyL1Managers[index].getProxyPort()
							+ " and DSO port as "
							+ proxyL1Managers[index].getDsoPort());
	...
	private synchronized void stopServerInternal(int index) throws Exception {
		...
		if (!expectedServerRunning[index]) {
			System.out
					.println("***Server not expected to be running. not stopping server ["
							+ serverControl[index].getDsoPort() + "]");
			return;
		}
		...
		if (index == activeIndex) {
			...
			stopL1Proxy(index);
			System.out.println("*** Server(active) stopped ["
					+ serverControl[index].getDsoPort() + "]");
			return;
		}
	...
	private void cleanupServerDB(int index) throws Exception {
		if (testConfig.getL2Config().getPersistenceMode()
				.equals(PersistenceMode.PERMANENT_STORE)) {
			System.out.println("Deleting data directory for server=["
					+ serverControl[index].getDsoPort() + "]");
	...
	private void deleteDirectory(String directory) {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				if (!successful) {
					throw new AssertionError("delete file=["
		...
		if (!(new File(directory).delete())) {
			throw new AssertionError("delete file=[" + directory + "] failed");
		}
		debugPrintln("\n ##### deleted directory=[" + directory + "]");
		debugPrintln("\n ##### dataFile=[" + directory + "] still exists? ["
				+ (new File(directory).exists()) + "]");
	}

	private TCServerInfoMBean getTcServerInfoMBean(int index)
			throws IOException {
		if (jmxConnectors[index] != null) {
			closeJMXConnector(index);
		}
		jmxConnectors[index] = getJMXConnector(serverControl[index]
				.getAdminPort());
		MBeanServerConnection mBeanServer = jmxConnectors[index]
				.getMBeanServerConnection();
		return MBeanServerInvocationHandler.newProxyInstance(mBeanServer,
				L2MBeanNames.TC_SERVER_INFO, TCServerInfoMBean.class, true);
	}
	...
	public DSOMBean getDsoMBean(int index) throws IOException {
		...
		DSOMBean dsoMBean = MBeanServerInvocationHandler.newProxyInstance(mbsc,
				L2MBeanNames.DSO, DSOMBean.class, false);
		return dsoMBean;
	}
	...
	public DGCMBean getLocalDGCMBean(int index) throws IOException {
		JMXConnector jmxc = getJMXConnector(serverControl[index].getAdminPort());
		MBeanServerConnection mbsc = jmxc.getMBeanServerConnection();
		DGCMBean dgcMBean = MBeanServerInvocationHandler.newProxyInstance(mbsc,
				L2MBeanNames.LOCAL_DGC_STATS, DGCMBean.class, false);
		return dgcMBean;
	}
	...
	private void closeJMXConnector(int i) {
		if (jmxConnectors[i] != null) {
			try {
				...
				System.out.println("JMXConnector for server=["
						+ serverControl[i].getDsoPort() + "] already closed.");
				e.printStackTrace();
			}
	...
	public synchronized void crashActiveAndWaitForPassiveToTakeOver()
			throws Exception {
		crashActiveInternal();

		if (expectedRunningServerCount() > 0) {
			debugPrintln("***** about to search for active  threadId=["
					+ Thread.currentThread().getName() + "]");
			updateActiveIndex();
			startL1Proxy(activeIndex);
		}
	...
	private void crashActiveInternal() throws Exception {
		...
		if (activeIndex < 0) {
			throw new AssertionError(
					"Active index was not set.No Active Server For Crashing");
		}

		System.out.println("Crashing active server: dsoPort=["
				+ serverControl[activeIndex].getDsoPort() + "]");
	...
	public synchronized void crashPassive(int passiveToCrash) throws Exception {
		...
		if (activeIndex == passiveToCrash) {
			throw new AssertionError(
					"Crash Passive Cannot crash active server["
							+ serverControl[passiveToCrash].getDsoPort() + "]");
		}

		System.out.println("Crashing passive server: dsoPort=["
				+ serverControl[passiveToCrash].getDsoPort() + "]");
		...
		if (!server.isRunning()) {
			throw new AssertionError("Server[" + server.getDsoPort()
					+ "] is not running as expected!");
		}
		server.crash();
	...
	public synchronized void crashRandomServer() throws Exception {
		...
		if (activeIndex < 0) {
			throw new AssertionError("Active index was not set.");
		}
		if (random == null) {
			throw new AssertionError("Random number generator was not set.");
		}
	...
	public void restartLastCrashedServer() throws Exception {
		...
		if (lastCrashedIndex >= 0) {
			if (serverControl[lastCrashedIndex].isRunning()) {
				throw new AssertionError("Server["
						+ serverControl[lastCrashedIndex].getDsoPort()
	...
	private void verifyActiveServerState() throws Exception {
		...
		if (jmxConnectors[activeIndex] == null) {
			jmxConnectors[activeIndex] = getJMXConnector(serverControl[activeIndex]
					.getAdminPort());
		}
		MBeanServerConnection mbs = jmxConnectors[activeIndex]
				.getMBeanServerConnection();
		TCServerInfoMBean mbean = MBeanServerInvocationHandler
				.newProxyInstance(mbs, L2MBeanNames.TC_SERVER_INFO,
	...
	public boolean dumpClusterState(int dumpCount, long dumpInterval)
			throws Exception {
		...
		if (serverControl[activeIndex].isRunning()) {
			System.out.println("Dumping server=["
					+ serverControl[activeIndex].getDsoPort() + "]");
			...
			try {
				...
				System.out.println("Need to recreate jmxConnector for server=["
						+ serverControl[activeIndex].getDsoPort() + "]...");
				jmxConnectors[activeIndex] = getJMXConnector(serverControl[activeIndex]
						.getAdminPort());
				mbs = jmxConnectors[activeIndex].getMBeanServerConnection();
			...
			L2DumperMBean mbean = MBeanServerInvocationHandler
					.newProxyInstance(mbs, L2MBeanNames.DUMPER,
			...
			try {
				...
				System.out
						.println("Could not find L2DumperMBean... sleep for 1 sec.");
				Thread.sleep(1000);
			}

			mbean.setThreadDumpCount(dumpCount);
			mbean.setThreadDumpInterval(dumpInterval);
			System.out.println("Thread dumping server=["
					+ serverControl[activeIndex].getDsoPort() + "] ");
			mbean.doThreadDump();
		...
		for (int i = 0; i < groupData.getServerCount(); i++) {
			try {
				...
				try {
					...
					System.out.println("exception restoring jmx connection ["
							+ e2.getMessage() + "]");
				}
			}
		}
	...
	public void startCrasher() {
		if (!testConfig.getCrashConfig().getCrashMode()
				.equals(ServerCrashMode.NO_CRASH)
				&& !testConfig.getCrashConfig().getCrashMode()
						.equals(ServerCrashMode.CUSTOMIZED_CRASH)) {
