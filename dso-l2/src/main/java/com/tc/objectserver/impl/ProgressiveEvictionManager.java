      ...
      // max out at 2 min.
        ...
        try {
            ...
            if ( monitored.getType() == MonitoredResource.Type.OFFHEAP ) {
                monitored.addReservedThreshold(MonitoredResource.Direction.RISING, monitored.getTotal() - 32l * 1024 * 1024, rb);
            }
        ...
        while ( !list.isEmpty() ) {
            ...
            push.add(agent.submit(new Callable<SampledRateCounter>() {
                ...
                public SampledRateCounter call() throws Exception {
                    EvictionTrigger triggerLocal = (pre) ? new BrakingEvictionTrigger(mapID , blowout) : new EmergencyEvictionTrigger(objectManager, mapID , blowout);
    ...
    // private long brake = 0;
        ...
        public void resourcesUsed(DetailedMemoryUsage usage) {
            try {
                ...
                if ( evictor.isLogging() ) {
                    if ( max != 0 ) {
                        log("Percent usage:" + (usage.getUsedMemory()*100/max) + " reserved:" + (reserve*100/max) + " time:" + (current - last) + " msec. threshold:" + threshold);
                    }
                }
...
//                        brake = usage.getReservedMemory();
 ...
 //  if we are this low, stop no matter what
            if ( usage.getReservedMemory() >= usage.getMaxMemory() - (16l * 1024 * 1024) ) {
                stop(usage);
            } else if ( usage.getReservedMemory() >= usage.getMaxMemory() - (64l * 1024 * 1024) && usage.getUsedMemory() >= usage.getMaxMemory() - (64l * 1024 * 1024)) {
                throttle(usage,1f);
            }
    ...
    */
        ...
        private void stop(MemoryUsage reserved) {
            ...
            isStopped = true;
            resourceManager.setThrowException();
            TerracottaOperatorEvent event = TerracottaOperatorEventFactory.createFullResourceCapacityEvent("pool",reserved.getUsedPercentage());
            TerracottaOperatorEventLogging.getEventLogger().fireOperatorEvent(event);
        }
        
        public void clear(MemoryUsage reserved) {
            if ( throttle == 0f && !isStopped ) {
                return;
            }
            isStopped = false;
            throttle = 0f;
