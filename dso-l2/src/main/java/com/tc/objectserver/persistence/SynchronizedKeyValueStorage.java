...
/**
 * Implements a KeyValueStorage interface on top of a provided flush routine which allows all writes to be synchronized
 * into a single store.  This means that the entire store of which it is a part will be read or written, at once, which is
 * slow but also means that they represent an atomic snapshot of data.
 * NOTE:  This is just temporary until IPersistentStorage can be fully removed.
 */
@CommonComponent
public class SynchronizedKeyValueStorage<K, V> implements KeyValueStorage<K, V>, StateDumpable, Serializable {
  private static final long serialVersionUID = -72159945771915614L;

  private final HashMap<K, V> storage;
  private transient FlushingWrite doFlush;
  
  /**
   * This constructor is only used in the case of serialization.
   */
  public SynchronizedKeyValueStorage() {
    this.storage = null;
    this.doFlush = null;
  }
   ...
   */
  public SynchronizedKeyValueStorage(FlushingWrite doFlush) {
   ...
   */
  public void setFlushCallback(FlushingWrite doFlush) {
