...
public class ObjectDataTestApp extends AbstractTransparentApp {
  ...
  protected final List<TestObject> workQueue         = new ArrayList<TestObject>();
        ...
        for (int i = 0; i < iterationCount; i++) {
          synchronized (resultSet) {
            ...
            if (i != (iterationCount - 1)) {
              for (Iterator<TestObject> iter = resultSet.iterator(); iter.hasNext();) {
  ...
  protected final class WorkerFactory {
      ...
      public void run() {
        ...
        try {
          while (true) {
            ...
            if (to.getId().equals("STOP")) return;
    ...
    public Runnable newWorker() {
      ...
      return new Worker("(" + appId + ") : Worker " + globalWorkerID + "," + ++localWorkerCount);
    }
