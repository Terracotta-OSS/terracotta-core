...
public class OnceAndOnlyOnceProtocolNetworkLayerImpl extends AbstractMessageTransport implements
  ...
  public void receive(TCByteBuffer[] msgData) {
    ...
    if (msg.isSend() || msg.isAck()) {
      ...
      Assert.assertTrue(isClient);
      Assert.assertTrue(handshakeMode.get());
  ...
  @Override
  public NetworkStackID open(InetSocketAddress serverAddress) throws TCTimeoutException, UnknownHostException, IOException,
      MaxConnectionsExceededException, CommStackMismatchException {
    Assert.assertNotNull(sendLayer);
    return sendLayer.open(serverAddress);
  }
